...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TokenWhale(props) {
			...
			/* Spout driven by token RATE: sqrt curve for visible low-rate response,
			 * saturating near ~50 tokens/second. Idle (rate 0) means no spout. */
			var level = rate <= 0 ? 0 : Math.min(1, Math.sqrt(rate / 50));
